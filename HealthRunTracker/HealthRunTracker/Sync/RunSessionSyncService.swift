...
final class RunSessionSyncService {
            ...
            if let error = error {
                if allowsRetry, self.shouldRetryOnHostedAPI(for: error) {
                    self.performDataRequest(
                        using: self.hostedFallbackBaseURL,
                        allowsRetry: false,
    ...
    func fetchLatestSessionStartTime(completion: @escaping (Result<Date?, Error>) -> Void) {
        performDataRequest(using: baseURL, allowsRetry: true, buildRequest: { requestBaseURL in
            var components = URLComponents(string: "\(requestBaseURL)/api/run-sessions/latest")
    ...
    func upload(_ session: RunSession, completion: ((Result<String, Error>) -> Void)? = nil) {
        ...
        performDataRequest(using: baseURL, allowsRetry: true, buildRequest: { requestBaseURL in
            guard let url = URL(string: "\(requestBaseURL)/api/run-session") else { return nil }

            var req = URLRequest(url: url)
            req.httpMethod = "POST"
            req.timeoutInterval = 20
            req.setValue("application/json", forHTTPHeaderField: "Content-Type")
            self.applyImportToken(to: &req)
    ...
    func uploadBatch(
        ...
        performDataRequest(using: baseURL, allowsRetry: true, buildRequest: { requestBaseURL in
            guard let url = URL(string: "\(requestBaseURL)/api/run-sessions/batch") else { return nil }
